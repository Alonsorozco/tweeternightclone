class Api::V1::TweetsController < ActionController::API  
  ...
  def date_filter
    ...
    @tweets = Tweet.includes(:likes).where("created_at >= ? AND created_at <= ?", params[:date1].to_date.beginning_of_day, params[:date2].to_date.end_of_day)
    render json: @tweets  
  ...
  def create
    ...
    @tweets =  User.first.tweet.create(content: params[:content])
    render json: @tweets 
